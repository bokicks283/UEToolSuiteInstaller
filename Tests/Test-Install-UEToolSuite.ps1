...
$testGlobalVersionRoot = Join-Path $testGlobalCliRoot "versions\1.0.4"
...
'@
  ...
  $globalVersionRoot = Join-Path $globalCliRoot "versions\1.0.4"
  ...
  Assert-Condition "case8 project marker declares the versioned bootstrap source" `
    ([string]$projectMarkerA.bootstrap.repositoryUrl -eq "https://github.com/bokicks283/UEToolSuiteInstaller.git" -and [string]$projectMarkerA.bootstrap.releaseTag -eq "v1.0.4") `
  ...
  Assert-Condition "case8 descriptor records payload version" ([string]$currentDescriptor.version -eq "1.0.4") "version=1.0.4" "version=$([string]$currentDescriptor.version)"
  ...
  Assert-PathExists "case8 teammate receives a separate versioned runtime" (Join-Path $teammateGlobalRoot "versions\1.0.4\Scripts\ue-tools.ps1")
