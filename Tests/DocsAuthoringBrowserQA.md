...
10. Drag a root page into the temporary section and verify:
    - the page appears nested in the sidebar
    - the public URL still loads through the preserved slug
    - no `DocsRoot`, transform, save helper, or fetch error is visible
11. Drag a root section into another section and verify the nested section appears under the target section.
12. Clean up all temporary docs files and reload `/docs/` to confirm the sidebar is back to the expected state.
...
- before/after file path for reorder failures
