...
- Project-file regeneration applies provenance-aware VS Code workspace overlays and restores pre-regen `.ignore` content. Run `ue settings adopt` once for a pre-existing customized workspace, then use `ue settings capture` for explicit ownership. See [`ue settings`](./CLI/Settings.md).
