...
'@
  ...
  # Legacy moves normalize section metadata and rewrite references, which can
  # legitimately take longer than the generic API smoke-test timeout on busy hosts.
  $legacySectionMoveTimeoutSec = 30
...
'@
    ...
    try {
      ...
      if ($legacySectionApiHostReady) {
        ...
        $legacyMove = Invoke-RestMethod -Uri "http://127.0.0.1:$legacySectionPort/api/move" -Method Post -ContentType "application/json" -Body $legacyMoveBody -TimeoutSec $legacySectionMoveTimeoutSec
        ...
        $legacyMoveBack = Invoke-RestMethod -Uri "http://127.0.0.1:$legacySectionPort/api/move" -Method Post -ContentType "application/json" -Body $legacyMoveBackBody -TimeoutSec $legacySectionMoveTimeoutSec
