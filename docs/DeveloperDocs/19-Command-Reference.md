...
|---|---|---|---|
| Build GUI installer EXE | repo root | `pwsh -NoProfile -ExecutionPolicy Bypass -File .\Scripts\Publish-InstallerExe.ps1 -Version 1.0.4` | Publishes GUI EXE under `dist/` |
| Validate local release preflight | repo root | `pwsh -NoProfile -ExecutionPolicy Bypass -File .\Scripts\Publish-GitHubRelease.ps1 -Version 1.0.4 -ValidateOnly` | Fetches `origin/main`; does not test, build, tag, or publish |
| Publish GitHub Release locally | repo root | `pwsh -NoProfile -ExecutionPolicy Bypass -File .\Scripts\Publish-GitHubRelease.ps1 -Version 1.0.4` | Runs release gates, builds installer, pushes annotated tag, and creates release |
