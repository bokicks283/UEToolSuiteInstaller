...
```text
dist/UEToolSuiteInstaller-1.0.5-win-x64.exe
```
...
The command runs the full non-mutating suite, both exclusive release suites, builds the installer, creates and pushes annotated tag `v1.0.5`, and creates the GitHub Release with the versioned installer asset. An existing tag may be resumed only when it already points to the current release commit; published releases and tags pointing elsewhere are never overwritten.
